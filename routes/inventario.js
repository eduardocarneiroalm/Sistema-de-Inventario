...
router.post("/adicionar", async (req, res) => {
  ...
  try {
    ...
    const buscarLocal = `SELECT id_locais FROM locais WHERE nome_locais = $1`;
    const resultadoLocal = await pool.query(buscarLocal, [local]);

    let idLocal;

    if (resultadoLocal.rows.length > 0) {
      idLocal = resultadoLocal.rows[0].id_locais;
    } else {
      // Criar local
      const criarLocal = `INSERT INTO locais (nome_locais) VALUES ($1) RETURNING id_locais`;
      const resultadoCriar = await pool.query(criarLocal, [local]);
      idLocal = resultadoCriar.rows[0].id_locais;
    }
    ...
    const inserirProduto = `
      ...
      VALUES ($1, $2, $3, $4)
    `;
    await pool.query(inserirProduto, [equipamento, "Sem descrição", quantidade, idLocal]);
...
router.put("/editar/:id", async (req, res) => {
  ...
  try {
    // Buscar id_local
    const sqlLocal = "SELECT id_local FROM produtos WHERE id_produtos = $1";
    const resultado = await pool.query(sqlLocal, [id]);
    const idLocal = resultado.rows[0].id_local;

    // Atualizar local
    const sqlUpdateLocal = "UPDATE locais SET nome_locais = $1 WHERE id_locais = $2";
...
router.delete("/deletar/:id", async (req, res) => {
  ...
  try {
    const sql = `DELETE FROM produtos WHERE id_produtos = $1`;
